 ...
 * @param[in]      Mode:   电机工作模式（1.运动模式Motion_mode 2. 位置模式Position_mode 3. 速度模式Speed_mode 4.
 * 电流模式Current_mode）
 * @retval         none
 */
 ...
 * @param[in]      mode: 电机工作模式（0.运动模式Motion_mode 1. 位置模式Position_mode 2. 速度模式Speed_mode 3.
 * 电流模式Current_mode）
 * @retval         none
 */
...
void motor_controlmode(MI_Motor* Motor, float torque, float MechPosition, float speed, float kp, float kd)
{
    ...
    txMsg.ExtId = Communication_Type_MotionControl << 24 | float_to_uint(torque, T_MIN, T_MAX, 16) << 8 |
