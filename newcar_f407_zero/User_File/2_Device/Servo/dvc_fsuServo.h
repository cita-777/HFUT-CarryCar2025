...
#ifndef DVC_FSUSERVO_H
#define DVC_FSUSERVO_H

#include <stdint.h>

#include "lib/FashionStar_UartServoProtocol.h"
...
public:
    ...
    bool ping();
    // 舵机标定
    void calibration(FSUS_SERVO_ANGLE_T rawA, FSUS_SERVO_ANGLE_T realA, FSUS_SERVO_ANGLE_T rawB,
    ...
    // 设置舵机的原始角度(指定周期)
    void setRawAngleByInterval(FSUS_SERVO_ANGLE_T rawAngle, FSUS_INTERVAL_T interval, FSUS_INTERVAL_T t_acc,
                               FSUS_INTERVAL_T t_dec, FSUS_POWER_T power);
    // 设定舵机的原始角度(指定转速)
    void setRawAngleByVelocity(FSUS_SERVO_ANGLE_T rawAngle, FSUS_SERVO_SPEED_T velocity, FSUS_INTERVAL_T t_acc,
                               FSUS_INTERVAL_T t_dec, FSUS_POWER_T power);
    // 设定舵机的原始角度(多圈)
    ...
    // 设定舵机的原始角度(多圈+指定周期)
    void setRawAngleMTurnByInterval(FSUS_SERVO_ANGLE_T rawAngle, FSUS_INTERVAL_T_MTURN interval, FSUS_INTERVAL_T t_acc,
                                    FSUS_INTERVAL_T t_dec, FSUS_POWER_T power);
    // 设定舵机的原始角度(多圈+指定转速)
    void setRawAngleMTurnByVelocity(FSUS_SERVO_ANGLE_T rawAngle, FSUS_SERVO_SPEED_T velocity, FSUS_INTERVAL_T t_acc,
                                    FSUS_INTERVAL_T t_dec, FSUS_POWER_T power);
    // 查询舵机的电压(单位mV)
