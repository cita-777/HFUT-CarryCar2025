...
typedef enum
{
    FSUS_BAUDRATE_9600   = 9600,
    FSUS_BAUDRATE_19200  = 19200,
    FSUS_BAUDRATE_38400  = 38400,
    FSUS_BAUDRATE_57600  = 57600,
    FSUS_BAUDRATE_115200 = 115200,
    FSUS_BAUDRATE_250000 = 250000,
    FSUS_BAUDRATE_500000 = 500000
} FSUS_BaudRate;

#define FSUS_DEFAULT_BAUDRATE FSUS_BAUDRATE_115200

// FSUS通信协议
// 注: FSUS是Fashion Star Uart Servo的缩写

// 串口通讯超时设置
#define FSUS_TIMEOUT_MS 200
...
#define FSUS_CMD_SET_ORIGIN_POINT 23              // 设置原点
// FSUS状态码
...
public:
    ...
    // 构造体
    FSUS_Protocol(UART_HandleTypeDef* huart, uint32_t baudrate);
    ...
    // 发送旋转的请求包(指定周期)
    void sendSetAngleByInterval(FSUS_SERVO_ID_T servoId, FSUS_SERVO_ANGLE_T angle, FSUS_INTERVAL_T interval,
                                FSUS_INTERVAL_T t_acc, FSUS_INTERVAL_T t_dec, FSUS_POWER_T power);
    // 发送旋转的请求包(指定转速)
    void sendSetAngleByVelocity(FSUS_SERVO_ID_T servoId, FSUS_SERVO_ANGLE_T angle, FSUS_SERVO_SPEED_T velocity,
                                FSUS_INTERVAL_T t_acc, FSUS_INTERVAL_T t_dec, FSUS_POWER_T power);
    // 发送舵机角度查询指令
    ...
    // 发送旋转的请求包(多圈)
    void sendSetAngleMTurn(FSUS_SERVO_ID_T servoId, FSUS_SERVO_ANGLE_T angle, FSUS_INTERVAL_T_MTURN interval,
                           FSUS_POWER_T power);
    // 发送旋转的请求包(多圈+指定周期)
